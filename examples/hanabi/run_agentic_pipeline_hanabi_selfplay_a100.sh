...
RAY_TMP_DIR="/workspace/MARS/ray_tmp"
mkdir -p $RAY_TMP_DIR
export RAY_TMP_DIR
...
ROLL_OUTPUT_DIR="/workspace/MARS/runs/hanabi_selfplay/$(date +%Y%m%d-%H%M%S)"
...
python examples/start_agentic_pipeline.py --config_path $CONFIG_PATH  --config_name agentic_val_hanabi_selfplay_a100 | tee $ROLL_LOG_DIR/custom_logs.log
